...
import { ArrowLeft, IdCard, ScrollText, HeartPulse, Wallet, Megaphone, Phone, MessagesSquare } from 'lucide-react'
...
type Props = {
  ...
  active: 'details' | 'prompt' | 'calls' | 'messages' | 'campaigns' | 'health' | 'costs'
}
...
export default function AdminClientHeader({ id, name, email, plan, planStatus, hasAssistant, active }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Tabs — horizontally scrollable so they never get crushed unreadable on a phone */}
      <div className="flex gap-1 overflow-x-auto" style={{ borderBottom: '1px solid var(--border)' }}>
        {[
          ...
          { key: 'calls' as const,    href: `/admin/clients/${id}/calls`,    label: 'Calls',                 icon: Phone      },
          { key: 'messages' as const, href: `/admin/clients/${id}/messages`, label: 'Messages',               icon: MessagesSquare },
