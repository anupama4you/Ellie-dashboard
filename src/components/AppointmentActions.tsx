  ...
  return (
    <>
      <div className="flex items-center gap-1 shrink-0">
        <button onClick={() => openModal('edit')} className="w-7 h-7 rounded-lg flex items-center justify-center btn-ghost" style={{ color: 'var(--ink-3)' }} title="Edit details">
          <Pencil size={12} />
        </button>
        <button onClick={() => openModal('reschedule')} className="w-7 h-7 rounded-lg flex items-center justify-center btn-ghost" style={{ color: 'var(--ink-3)' }} title="Reschedule">
          <CalendarClock size={13} />
        </button>
        <button onClick={() => openModal('cancel')} className="w-7 h-7 rounded-lg flex items-center justify-center btn-ghost" style={{ color: 'var(--coral)' }} title="Cancel appointment">
          <XCircle size={13} />
        </button>
      </div>
            ...
            {smsWarning ? (
              <>
                <p className="text-sm" style={{ color: 'var(--ink-2)' }}>
                  {modal === 'reschedule' ? 'The appointment was moved.' : 'The appointment was cancelled.'}
                </p>
                <div className="rounded-lg px-3 py-2" style={{ background: 'var(--amber-soft)' }}>
                  <p className="text-xs font-semibold" style={{ color: 'var(--amber)' }}>Confirmation text not sent</p>
                  <p className="text-xs mt-0.5" style={{ color: 'var(--amber)' }}>{smsWarning}</p>
                </div>
                <button onClick={close} className="w-full rounded-xl py-2.5 text-sm font-bold text-white mt-1 transition-opacity hover:opacity-90"
